...
playerName,
  ...
  if ((d.month == 2 and d.day == 13) or (d.month == 2 and d.day == 14)) then
    ...
    table.insert(
      rudeLines,
      "I hope you get a box of chocolates with a toothpick in it, ${targetName}."
    )
  end
  ...
  if ((d.month == 4 and d.day == 4) or (d.month == 4 and d.day == 5)) then
    ...
    table.insert(
      rudeLines,
      "I hope you get a chocolate bunny with a bite taken out of it, ${targetName}."
    )
  end
  ...
  if ExpressYourElfVars.allowYoMamaContent then
    table.insert(
      rudeLines,
      "Yo mama's so fat, when she fell off the Lich King's throne, she got a 10"
    )
    table.insert(
      rudeLines,
    ...
    if (playerClass == Class.Shaman) then
      ...
      table.insert(
        rudeLines,
        "Yo mama so fat, my Earth Shield ran out before it made a single orbit"
      )
    end
    ...
    if (playerClass == Class.Warlock) then
      ...
      table.insert(
        rudeLines,
        "Yo mama so fat, I had to grease the portal and conjure some mana cakes on the other side just to get her through."
      )
    end
  end
  ...
  if #parsedLine > 254 then
    ...
    return ExpressYourElf.IAmRude.GetMessage(
      playerName,
      playerGender,
      playerClass,
      playerRace,
...
function ExpressYourElf.IAmRude.run()
  if (UnitName("target") and UnitPlayerControlled("target")) then
    ...
    local rudeLine =
      ExpressYourElf.IAmRude.GetMessage(
        playerName,
        playerGender,
        playerClass,
        playerRace,
