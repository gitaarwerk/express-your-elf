...
function ExpressYourElf.DanceWithMe.getPlayerMessages(name, gender, class, race, level, guyGirl)
    ...
    if (ExpressYourElfVars.allowMatureContent) then
        for _, item in ipairs(ExpressYourElf.DanceWithMe.matureList) do
            table.insert(ExpressYourElf.DanceWithMe.list, item)
        end
    end
...
function ExpressYourElf.DanceWithMe.getTargetMessages(name, gender, class, race, level, hisHer, heShe, guyGirl,
    ...
    if (ExpressYourElfVars.allowMatureContent) then
        for _, item in ipairs(ExpressYourElf.DanceWithMe.matureListWithTarget) do
            table.insert(ExpressYourElf.DanceWithMe.listWithTarget, item)
        end
    end
...
function ExpressYourElf.DanceWithMe.run()
    ...
    if (UnitName("target") and UnitPlayerControlled("target")) then
        ...
        local targetHisHer                                                   = ExpressYourElf.Helpers.GetHisHer(
            targetGender)
        local targetHeShe                                                    = ExpressYourElf.Helpers.GetHeShe(
            targetGender)
        local targetGuyGirl                                                  = ExpressYourElf.Helpers.GetGuyGirl(
            targetGender)
        local oppositeBfGf                                                   = ExpressYourElf.Helpers.oppositeBfGf(
            targetGender)

        local targetMessages                                                 = ExpressYourElf.DanceWithMe
            .getTargetMessages(targetName, targetGender, targetClass, targetRace, targetLevel, targetHisHer, targetHeShe,
                targetGuyGirl, oppositeBfGf)

        SendChatMessage(targetMessages[fastrandom(1, #targetMessages)], "SAY", nil, index);
    else
        local playerMessages = ExpressYourElf.DanceWithMe.getPlayerMessages(playerName, playerGender, playerClass,
            playerRace, playerLevel, playerGuyGirl)
