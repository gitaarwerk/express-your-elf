...
playerName,
  ...
  if (playerClass == Class.Shaman or playerClass == Class.Druid) then
    table.insert(
      jokeLines,
  ...
  if (playerClass == Class.Monk or playerClass == Class.Priest or playerClass == Class.Paladin) then
    table.insert(
      jokeLines,
  ...
  if (playerClass == Class.Warrior) then
    ...
    table.insert(
      jokeLines,
      "I'm a Warrior. I can shout. I can scream. I can do a lot of things. But I can't dance."
    )
  ...
  if (playerClass == Race.Dracthyr) then
    table.insert(
      jokeLines,
  ...
  if (playerRace == Race.Undead) then
    ...
    table.insert(
      jokeLines,
      "What did the Forsaken say to the Silvermoon street walker? You can keep the tip."
    )
    table.insert(jokeLines, "What do you call a forsaken who likes bad jokes? Pundead")
  end

  if ((playerRace == Race.Gnome or playerRace == Race.Goblin) and playerClass == Class.Priest) then
  ...
  if ExpressYourElfVars.allowMatureContent then
    ...
    table.insert(
      jokeLines,
      "How do you know you play too much WoW? When your partner's pants have a low drop rate."
    )
  ...
  if (ExpressYourElfVars.allowMatureContent and ExpressYourElfVars.bringBackRemoved) then
    ...
    if (playerRace == Race.BloodElf and playerGender == "female" and playerClass == Class.DemonHunter) then
      table.insert(jokeLines, "Of course these are real. You...mean the tattoos, right?")
    end

    if (playerRace == Race.BloodElf and playerGender == "male") then
    ...
    if (playerRace == Race.Orc and playerGender == "female") then
      table.insert(jokeLines, "What's estrogen? Can you eat it?")
    end

    if (playerRace == Race.Tauren and playerGender == "female") then
      table.insert(jokeLines, "One time I laughed so hard I milked all over the floor.")
    end

    if (playerRace == Race.Tauren and playerGender == "male") then
    ...
    if (playerRace == Race.Gnome and playerGender == "male") then
      table.insert(
        jokeLines,
    ...
    if (playerRace == Race.Human and playerGender == "female") then
      table.insert(
        jokeLines,
    ...
    if (playerRace == Race.Human and playerGender == "male") then
      table.insert(
        jokeLines,
    ...
    if (playerRace == Race.Mechagnome and playerGender == "male") then
      table.insert(
        jokeLines,
  ...
  local parsedLine = ExpressYourElf.Helpers.parseText(pickedLine, {
  ...
  if #parsedLine > 254 then
    ...
    return ExpressYourElf.Joke.GetMessage(
      playerName,
      playerGender,
      playerClass,
      playerRace,
...
function ExpressYourElf.Joke.run()
  ...
  local JokeLine =
    ExpressYourElf.Joke.GetMessage(
      playerName,
      playerGender,
      playerClass,
      playerRace,
  ...
  local emotes = { "CACKLE", "CHUCKLE", "GIGGLE", "LAUGH", "COUGH", "GRIN", "ROFL", "WICKED" }
