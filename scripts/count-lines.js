...
const features = [
  ...
  { name: "Seduce", tableName: "seduceLines" },
];
...
function countFeatureLines({ name: featureName, tableName }) {
  ...
  const insertPattern = new RegExp(`table\\.insert\\(\\s*${tableName}\\s*,`, "g");
...
function writeStats() {
  ...
  fs.writeFileSync(
    jsonFile,
    JSON.stringify(
      { features: stats, total: totalLines, generated: new Date().toISOString() },
      null,
      2
    )
  );
